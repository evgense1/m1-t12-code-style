...
public class DepositCalculator {
    double calculateComplexPercent(double amount, double yearRate, int depositPeriod) {
        ...
        return calculateTotalAmount(pay, 2);
    }

    double calculateSimplePercent(double amount, double yearRate, int depositPeriod) {
        return calculateTotalAmount(amount + amount * yearRate * depositPeriod, 2);
    }
    double calculateTotalAmount(double value, int places) {
    ...
    void calculateDepositIncome() {
        ...
        System.out.println("Результат вклада: "
                + amount + " за " + period + " лет превратятся в " + totalAmount);
    }
